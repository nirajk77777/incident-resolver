...
| `GITHUB_PERSONAL_ACCESS_TOKEN` | The GitHub MCP server the Fix Shipper opens pull requests through | A confirmed code bug escalates with the RCA on the timeline and the patch in the Workspace, rather than becoming a PR. The Fix Shipper is not built yet, so nothing reads this today |
...
`packages/agents` is the agent itself: the Resolver deep agent, its Triage subagent, three Investigators and Code RCA, the prompts, and a CLI that runs one Ticket and prints the Verdict.

- The Resolver is a Deep Agents JS agent on `gpt-5.4` with a Postgres checkpointer in the `portal` schema, one LangGraph thread per run. Triage and the three Investigators are subagents on `gpt-5.4-mini` and Code RCA is one on `gpt-5.4`, all reached through the `task` tool. Every model name comes from `packages/shared/src/config.ts`.
...
pass  double-discount   code_bug                    escalated                      0.74
FAIL  empty-cart-crash  unknown (want code_bug)     answered (want fix_proposed or escalated)  0.31
```
...
each with the Outcomes that count and why. Two of them accept `fix_proposed` **or**
`escalated`: a confirmed code bug becomes a pull request once the Fix Shipper lands, and
until then escalates with the RCA on the timeline and the patch waiting in the Workspace, and
both are the agent behaving correctly for the code it is running.
...
line and the test. *Code RCA with no shell: two fixed commands, `run_tests` and
`git_diff_names`.*
