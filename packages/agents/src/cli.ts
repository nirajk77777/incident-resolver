 ...
 * OPENAI_API_KEY and COHERE_API_KEY. The Langfuse keys are optional and enable tracing. Keys
 * are secrets and come straight from the environment; every tunable comes from config.
 */
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const resolver = createResolver({ config, models, tools, checkpointer });
    ...
    const report = await traceRun(ticket, config.models, (callbacks) =>
